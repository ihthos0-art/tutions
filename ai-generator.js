(function () {
  ...
  async function generateELA(cfg) {
    ...
    var reply = await groqChat([systemPrompt, userPrompt], 1500);
    ...
    try {
      ...
      console.error('[ai-generator] Failed to parse ELA response:', e, reply);
      return null;
    }
  }
  ...
  function buildMathHTML(problems, pattern) {
    ...
    if (pattern === 'checked') {
      ...
      // Pattern A: items to go inside .math-grid — no wrapper (caller owns the container)
      ...
      // span full grid width so word problems don't get squeezed into one column
      for (var i = 0; i < words.length; i++) {
        html += '<div class="math-word" style="grid-column:1/-1">' + (drill.length + i + 1) + '. ' + escapeHtml(words[i].text) + ' <input type="text" data-save="m1-' + (drill.length + i + 1) + '" placeholder="?" /></div>';
      }
    }
    return html;
  }
